...
export default function Home() {
  ...
  const abrirAreaProtegida = (destino: string) => {
    if (isAuthenticated) {
      irPara(destino);
      return;
    }
  ...
  return <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
    ...
    <header className="border-b border-border bg-background/90 backdrop-blur-md sticky top-0 z-30"><div className="container h-18 min-h-16 flex items-center justify-between gap-3"><LogoInsider className="w-28 sm:w-36" /><div className="flex items-center gap-2 sm:gap-3"><span className="hidden md:inline label-ed">Sistema de Parcerias</span><ThemeToggle /><Button size="sm" variant="outline" className="h-10 bg-card hover:bg-accent" disabled={loading} onClick={abrirCadastro}> {loading ? "Verificando acesso" : isAuthenticated ? "Meu cadastro" : "Entrar"}</Button></div></div></header>
    <main id="conteudo-principal" tabIndex={-1}>
      <section className="relative overflow-hidden border-b border-border"><div className="container relative z-10 pt-16 pb-16 lg:pt-24 lg:pb-24"><div className="max-w-4xl"><div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-primary/10 text-primary text-xs font-semibold uppercase tracking-wider"><Sparkles className="size-3.5" />Encontro Insider · Rodada de Parcerias</div><h1 className="font-display text-4xl sm:text-5xl lg:text-7xl font-bold tracking-tight mt-6 leading-[1.05]">Projetos validados.<br /><span className="text-primary">Conversas que avançam.</span></h1><p className="max-w-2xl text-base sm:text-lg text-muted-foreground mt-7 leading-relaxed">A central que organiza Experts, Lançadores e projetos preparados para a Rodada de Parcerias. O cadastro é aberto à comunidade e a operação aprova cada participação antes de liberar o painel.</p><div className="mt-9 flex flex-col sm:flex-row gap-3"><Button size="lg" className="gap-2" onClick={abrirCadastro}>Participar da Rodada <ArrowRight className="size-4" /></Button><Button size="lg" variant="outline" className="bg-card hover:bg-accent" onClick={() => irPara("/cadastro")}>Como funciona o cadastro</Button></div></div></div><svg aria-hidden="true" viewBox="0 0 560 420" className="pointer-events-none absolute right-0 top-0 hidden h-full w-[48%] min-w-[480px] lg:block" fill="none"><path d="M558 52C436 48 401 100 292 210" stroke="#718AB5" strokeWidth="2" opacity=".5" /><path d="M558 156C442 154 397 172 292 210" stroke="#3A5399" strokeWidth="2" opacity=".55" /><path d="M558 270C432 274 399 244 292 210" stroke="#718AB5" strokeWidth="2" opacity=".5" /><path d="M558 374C431 362 387 292 292 210" stroke="#3A5399" strokeWidth="2" opacity=".45" /><circle cx="292" cy="210" r="9" fill="#3A5399" opacity=".85" /><circle cx="292" cy="210" r="19" stroke="#718AB5" strokeWidth="1.5" opacity=".55" /></svg></section>
    ...
    <footer className="border-t border-border"><div className="container py-6 flex flex-col sm:flex-row gap-2 justify-between text-xs text-muted-foreground"><span>FL Insider · Sistema de Parcerias</span><span>Inscrição sujeita à aprovação da operação do evento</span></div></footer>
  </div>;
}
