...
| **Parcialmente concluído** | A sessão OAuth passou de um ano para 12 horas; não há MFA obrigatório nem invalidação após alteração de privilégio. | A janela de exposição foi reduzida, mas Administradores ainda dependem do encerramento/expiração para revogar uma sessão existente. | Implementar invalidação e renovação controlada após mudança de privilégio e definir MFA administrativo. |
...
| Renovação e invalidação após alteração de privilégio | **Pendente** |
